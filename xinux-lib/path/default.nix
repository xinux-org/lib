...
in {
  path = rec {
    ...
    in
      assert assertMsg (match != null) "lib.xinux.split-file-extension: File must have an extension to split."; match;
    ...
    ## ```
    #@ Path -> Path
    get-parent-directory = xinux-lib.fp.compose baseNameOf dirOf;
