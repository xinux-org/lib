...
in
{
  module = {
    ...
    create-modules =
      ...
      let
        user-modules = xinux-lib.fs.get-default-nix-files-recursive src;
          ...
          // {
            ...
            ${metadata.name} =
              ...
              let
                ...
                format =
                  let
                    virtual-system-type = xinux-lib.system.get-virtual-system-type target;
                  in
                  if virtual-system-type != "" then
                    virtual-system-type
                  else if xinux-lib.system.is-darwin target then
                    "darwin"
                  else
                    "linux";

                # Replicates the specialArgs from Xinux Lib's system builder.
                modified-args = args // {
                  inherit system target format;
                  virtual = args.virtual or (xinux-lib.system.get-virtual-system-type target != "");
